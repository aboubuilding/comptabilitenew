...
@endphp
                        ...
                        <table class="table-responsive-lg table display dataTablesCard student-tab dataTable no-footer" id="example">
                            ...
                            @foreach( $data as $encaissement  )
                                ...
                                <tr>
                                    ...
                                    <td><h6 class="mb-0">{{ $encaissement ['caisse'] }} </h6></td>

                                    @if( $role == \App\Types\Role::DIRECTEUR || $role == \App\Types\Role::ADMIN )
                                    <td><h6 class="mb-0">{{ $encaissement ['responsable'] }} </h6></td>
                                        ...
                                        <div class="d-flex">
                                            <div class="dropdown custom-dropdown ">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <a class="dropdown-item detailEncaissement" href="" data-id="{{$encaissement['id']}}">Voir détail </a>
                                                    <a class="dropdown-item" href="{{ route('encaissement_pdf', ['id' => $encaissement['id']]) }}">Imprimer</a>


                                                        <a class="dropdown-item demanderAnnulation" href="#"  data-id="{{$encaissement['id']}}">Demander annulation </a>
    ...
    @include('admin.encaissement.modaldetail')
...
@section('js')
    ...
    <script>
        jQuery(document).ready(function() {
            ...
            $(document).on('click', '.detailEncaissement', function(event) {


                event.preventDefault();
                let id = $(this).data('id');
                let url = "/encaissements/detail/" + id;
                ...
                $.get(url, function(data) {
                    ...
                    $('#defaultModalLabel').text('Détail d un encaissement  ');

                    let detail_modal = $('#detailEncaissement');

                    $(detail_modal).find('form').find('input[name="reference"]').val(data.paiement.reference);
                    $(detail_modal).find('form').find('input[name="beneficiaire"]').val(data.encaissement.beneficiaire);
                    $(detail_modal).find('form').find('input[name="montant"]').val(data.encaissement.montant);
                    $(detail_modal).find('form').find('input[name="caisse"]').val(data.encaissement.caisse.libelle);
                    $(detail_modal).find('form').find('input[name="responsable"]').val(data.responsable);
                    ...
                    $('#beneficiaire').prop('disabled', true);
                    ...
                    $('#idEncaissement').val(data.encaissement.id);
        ...
        function fermerDetail() {
            ...
            $('#detailEncaissement').modal('toggle');
        }
