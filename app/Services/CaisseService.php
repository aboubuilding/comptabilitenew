...
class CaisseService
{
    private const SEUIL_ECARD_NEGLIGEABLE = 100;
    private const SEUIL_ECARD_SIGNIFICATIF = 5000;
    ...
    public function getCurrentUser(): ?User
    {
        ...
        return $compteId ? $this->userRepo->find($compteId) : null;
    }
    ...
    public function createCaissier(array $data, ?User $creator = null): User
    {
        ...
        if ($this->userRepo->activeQuery()->where('login', $data['login'])->exists()) {
    ...
    {
        $query = $this->userRepo->activeQuery()
    ...
    public function deactivateCaissier(int $userId, ?User $admin = null): bool
    {
        ...
        $caissier = $this->userRepo->findOrFail($userId);
        throw_if((int) $caissier->role !== Role::CAISSIER, new LogicException('Cet utilisateur n\'est pas un caissier.'));

        return $this->userRepo->update($userId, ['etat' => TypeStatus::SUPPRIME]);
    }
    ...
    public function creerCaisse(array $data, ?User $user = null): \App\Models\Caisse
    {
        $user = $user ?? $this->getCurrentUser();
    ...
    public function ouvrirCaisse(int $caisseId, float $soldeInitial, ?User $user = null): \App\Models\Caisse
    {
        $user = $user ?? $this->getCurrentUser();
        throw_if(!$user, new LogicException('Utilisateur non authentifié.'));
    ...
    private function handleNegligeableEcart(\App\Models\Caisse $caisse, float $ecart, User $user): void
    {
        if (abs($ecart) > 0.01) {
            $this->createEcartMouvement($caisse, abs($ecart), $ecart > 0 ? 'sortie' : 'entree', $user->id);
        }
        $this->caisseRepo->update($caisse->id, [
    ...
    private function handleSignificatifEcart(\App\Models\Caisse $caisse, float $ecart, ?string $motif, User $user): void
    {
    ...
    private function handleCritiqueEcart(\App\Models\Caisse $caisse, float $ecart, ?string $motif, User $user): void
    {
        ...
        if (abs($ecart) > 0.01) {
            $this->createEcartMouvement($caisse, abs($ecart), $ecart > 0 ? 'sortie' : 'entree', $user->id, true);
        }
        $this->caisseRepo->update($caisse->id, [
            'ecart_constate'       => $ecart,
            'motif_ecart'          => $motif,
    ...
    private function createEcartMouvement(\App\Models\Caisse $caisse, float $montant, string $direction, int $userId, bool $isCritique = false): void
    {
        $typeMvt = $direction === 'sortie' ? TypeMouvement::DECAISSEMENTS : TypeMouvement::ENCAISSEMENT;
    ...
    public function getCaissesWithAggregates(array $filters = []): array
    {
        ...
        $aggregates = [
            ...
            'fonds_final_total'   => (float) $caisses->where('statut', StatutCaisse::CLOTURE)->sum('solde_final'),
        ];
    ...
    public function getCaisseDetail(int $id): ?array
    {
        ...
        $caisse = $this->caisseRepo->activeQuery()
            ->with(['responsable:id,nom,prenom,login', 'utilisateur:id,nom,prenom,login'])
    ...
    {
        return $this->mouvementRepo->activeQuery()
            ...
            ->map(fn($m) => [
                ...
                'motif'               => $m->motif,
                'date'                => $this->formatDateTime($m->date_mouvement),
                'effectue_par'        => $m->operateur?->nom . ' ' . $m->operateur?->prenom,
                'paiement_reference'  => $m->paiement?->reference,
                'paiement_libelle'    => $m->paiement?->libelle,
                'paiement_id'         => $m->paiement_id,
            ])->toArray();
    }
    ...
    private function getDecaissementsWithExpenseRef(int $caisseId, bool $isViewer): array
    {
        return $this->mouvementRepo->activeQuery()
            ...
            ->where('caisse_id', $caisseId)
            ->where('type_mouvement', TypeMouvement::DECAISSEMENTS)
            ->where('statut_mouvement', StatutMouvement::VALIDER)
            ->orderByDesc('date_mouvement')
            ->get()
            ->map(fn($m) => [
                ...
                'date'              => $this->formatDateTime($m->date_mouvement),
                'effectue_par'      => $m->operateur?->nom . ' ' . $m->operateur?->prenom,
                'depense_reference' => $m->depense?->reference,
                'depense_libelle'   => $m->depense?->libelle,
                'depense_id'        => $m->depense_id,
            ])->toArray();
    }

    private function formatCaisse(\App\Models\Caisse $c, bool $isViewer): array
    {
        return [
            'id'                 => $c->id,
            'libelle'            => $c->libelle,
            ...
            'statut'             => (int) $c->statut,
            'statut_label'       => $this->getStatutLabel((int) $c->statut),
            'annee_id'           => $c->annee_id,
    ...
    {
        return match($statut) {
            StatutCaisse::OUVERT   => 'Ouverte',
            StatutCaisse::CLOTURE  => 'Clôturée',
            default                => 'Inconnu'
        };
    }
    ...
    public function getEcartReport(array $filters = []): array
    {
        ...
        $query->addSelect(\Illuminate\Support\Facades\DB::raw("$dateGroup as periode"));
    ...
    public function getMouvementsByCaissier(int $caissierId, array $filters = []): array
    {
        ...
        $query = $this->mouvementRepo->activeQuery()
            ...
            ->when($type === 'sortie', fn($q) => $q->where('type_mouvement', TypeMouvement::DECAISSEMENTS))
            ->orderByDesc('date_mouvement');
        ...
        $mouvements = $query->get()->map(fn($m) => [
            ...
            'beneficiaire'      => $m->beneficiaire,
            'date'              => $this->formatDateTime($m->date_mouvement),
            'caisse_libelle'    => $m->caisse?->libelle,
