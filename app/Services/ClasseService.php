...
class ClasseService extends BaseService
{
    ...
    /**
     * Liste simplifiée pour selects (dropdown) avec filtres spécifiques.
     * Cette méthode remplace l'ancienne getForSelect non conforme.
     * Elle retourne une Collection d'objets, pas formatée.
     * Pour un format standardisé, utilisez plutôt la méthode parent (BaseService::getForSelect).
     */
    public function getSelectList(?int $anneeId = null, ?int $cycleId = null, ?int $niveauId = null): Collection
    {
        $query = $this->repo->activeQuery()
            ->where('etat', 1)
    ...
    /**
     * Vérifie si une classe a des données liées
     */
